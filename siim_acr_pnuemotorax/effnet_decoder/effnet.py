###
# picked from here https://github.com/lukemelas/EfficientNet-PyTorch/blob/master/efficientnet_pytorch/model.py
###
...
class EfficientNetEncoder(nn.Module):
    ...
    def __init__(self, blocks_args=None, global_params=None, output_indices=(4, 9, 12)):
        super().__init__()
        self.channels = set(output_indices)
    ...
    def from_name(cls, model_name, override_params=None):
        ...
        return EfficientNetEncoder(blocks_args, global_params)

    @classmethod
    def from_pretrained(cls, model_name, num_classes=1000):
        model = EfficientNetEncoder.from_name(model_name, override_params={'num_classes': num_classes})
        if model_name in effnet_channels.keys():
            model.channels = effnet_channels[model_name]
        else:
            print('model name', model_name, 'not supported, skipping channels')
        load_pretrained_weights_effnet(model, model_name, load_fc=(num_classes == 1000))
...
effnet_output_shapes = {
    'efficientnet-b0': [1280, 80, 40, 24, 32],
...
if __name__ == '__main__':
    model = EfficientNetEncoder.from_pretrained('efficientnet-b0')
