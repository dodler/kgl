...
class Decoder(nn.Module):
    def __init__(self, in_channels, middle_channels, out_channels, use_self_attention=True):
        super().__init__()
...
class UNet(nn.Module):
    def __init__(self, basenet='vgg11', num_filters=16, pretrained='imagenet'):
        ...
        self.encoder5 = nn.Sequential(
            ...
            ABN(context_channels, activation='relu'),
        ...
        self.center = Decoder(self.encoder5.out_channels,
                              num_filters * 8 * 2, num_filters * 8, use_self_attention=True)
        ...
        self.decoder4 = Decoder(self.encoder4.out_channels + num_filters * 8,
                                num_filters * 8 * 2, num_filters * 4, use_self_attention=True)
        self.decoder3 = Decoder(self.encoder3.out_channels + num_filters * 4,
                                num_filters * 4 * 2, num_filters * 2, use_self_attention=True)

        if basenet.startswith('vgg'):
            self.decoder2 = Decoder(self.encoder2.out_channels + num_filters * 2,
                                    num_filters * 2 * 2, num_filters, use_self_attention=True)
